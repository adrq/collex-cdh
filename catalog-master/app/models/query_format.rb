...
class QueryFormat
  ...
  #added this method for composition field -- akhil
  def self.transform_composition(key,val)
  ...
  def self.transform_last_modified(key, val)
	...
	def self.create_solr_query(format, params, request_ip)
		...
		if request_ip && !Federation.request_from_federation(request_ip)
			if params['o'].blank?
                params['o'] = '+freeculture' #adrian - re enable ip check // make sure that collex ip is set in catalog whitelist
			else
